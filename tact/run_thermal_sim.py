...
def run_thermal_sim(sonde, atmosphere="../data/atmmod/afglss.dat", obase=None,
    ...
    for i in range(3):
        ...
        if (override) or (os.path.exists(outfile) is False):
            ...
            cfg = tact.cfg_create(runfile=runfile, 
                             look_down=look_down,
            ...
            outfile = tact.libradtran_run_file(runfile)
        data[run] = tact.read_out(outfile,parameters = parameters)
    ...
    if True:
        ...
        if rsr_data is not None:
            for satsen in rsr_data:
                ...
                ret_wave = tact.rsr_convolute_dict(muwave,
                                                    muwave, 
                                                    rsr_data[satsen]['rsr'], 
                                                    wave_range=[9,14], wave_step=0.05)

                ret_tau = tact.rsr_convolute_dict(muwave, 
                                                     tau,
                                                     rsr_data[satsen]['rsr'], 
                                                     wave_range=[9,14], wave_step=0.05)

                ret_Lu = tact.rsr_convolute_dict(muwave, 
                                                     Lu,
                                                     rsr_data[satsen]['rsr'], 
                                                     wave_range=[9,14], wave_step=0.05)

                ret_Ld = tact.rsr_convolute_dict(muwave, 
                                                      Ld,
                                                      rsr_data[satsen]['rsr'],
                                                     wave_range=[9,14], wave_step=0.05)
