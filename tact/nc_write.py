...
##                QV 2019-07-04 added support for _FillValue in dataset_attributes
...
def nc_write(ncfile, dataset, data, wavelength=None, global_dims=None,
    ...
    if new:
        ...
        setattr(nc, 'auto_grouping', 'rhot:rhorc:rhos:rhow:Rrs:rhog:rhoe:rhose:rhou')
    ...
    if dataset in nc.variables.keys(): 
        ...
        if dataset_attributes is not None:
              if '_FillValue' in dataset_attributes: 
                  fill_value = dataset_attributes['_FillValue']
                  del dataset_attributes['_FillValue']

        ## new dataset
        var = nc.createVariable(dataset,data.dtype,('y','x'), zlib=nc_compression, chunksizes=chunksizes, fill_value=fill_value)
